...
func (c *Cleaner) Clean(ctx context.Context) error {
	...
	watcher := informers.WatcherSet[informers.VolumeSnapshot]
	...
	for _, item := range resources {
		...
		if ok {
			if config.Global.GarbageCollection.Debug {
				...
				err := config.SnapshotClient.VolumeSnapshots(item.GetNamespace()).Delete(ctx, item.GetName(), k8smetav1.DeleteOptions{
...
func (c *Cleaner) Mark(ctx context.Context, namespace string, timestamp string) error {
	...
	for _, item := range resources {
		...
		vs, err := utils.Transform[k8smetav1.Object, v1.VolumeSnapshot](item)
		if err != nil {
		...
		if _, err := config.SnapshotClient.VolumeSnapshots(item.GetNamespace()).Update(ctx, &vs, k8smetav1.UpdateOptions{}); err != nil {
