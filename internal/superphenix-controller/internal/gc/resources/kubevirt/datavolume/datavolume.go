...
func (c *Cleaner) Clean(ctx context.Context) error {
	...
	watcher := informers.WatcherSet[informers.DataVolume]
	...
	for _, item := range resources {
		...
		if ok {
			if config.Global.GarbageCollection.Debug {
				...
				err := config.VirtClient.CdiClient().CdiV1beta1().DataVolumes(item.GetNamespace()).Delete(ctx, item.GetName(), k8smetav1.DeleteOptions{
...
func (c *Cleaner) Mark(ctx context.Context, namespace string, timestamp string) error {
	...
	for _, item := range resources {
		...
		dv, err := utils.Transform[k8smetav1.Object, v1beta1.DataVolume](item)
		if err != nil {
		...
		if _, err := config.VirtClient.CdiClient().CdiV1beta1().DataVolumes(item.GetNamespace()).Update(ctx, &dv, k8smetav1.UpdateOptions{}); err != nil {
