...
func (c *Cleaner) Clean(ctx context.Context) error {
	...
	watcher := informers.WatcherSet[informers.VirtualMachineSnapshot]
	...
	for _, item := range resources {
		...
		if ok {
			if config.Global.GarbageCollection.Debug {
				...
				err := config.VirtClient.VirtualMachineSnapshot(item.GetNamespace()).Delete(ctx, item.GetName(), k8smetav1.DeleteOptions{
...
func (c *Cleaner) Mark(ctx context.Context, namespace string, timestamp string) error {
	...
	for _, item := range resources {
		...
		vmSnapshot, err := utils.Transform[k8smetav1.Object, v1beta1.VirtualMachineSnapshot](item)
		if err != nil {
		...
		if _, err := config.VirtClient.VirtualMachineSnapshot(item.GetNamespace()).Update(ctx, &vmSnapshot, k8smetav1.UpdateOptions{}); err != nil {
