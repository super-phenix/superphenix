...
func (c *Cleaner) Clean(ctx context.Context) error {
	...
	watcher := informers.WatcherSet[informers.PersistentVolumeClaims]
	...
	for _, item := range resources {
		...
		if ok {
			if config.Global.GarbageCollection.Debug {
				...
				err := config.K8sClient.CoreV1().PersistentVolumeClaims(item.GetNamespace()).Delete(ctx, item.GetName(), k8smetav1.DeleteOptions{
...
func (c *Cleaner) Mark(ctx context.Context, namespace string, timestamp string) error {
	...
	for _, item := range resources {
		...
		pvc, err := utils.Transform[k8smetav1.Object, v1.PersistentVolumeClaim](item)
		if err != nil {
		...
		if _, err := config.K8sClient.CoreV1().PersistentVolumeClaims(item.GetNamespace()).Update(ctx, &pvc, k8smetav1.UpdateOptions{}); err != nil {
