...
func (c *Cleaner) Clean(ctx context.Context) error {
	...
	watcher := informers.WatcherSet[informers.NetworkPolicy]
	...
	for _, item := range resources {
		...
		if ok {
			if config.Global.GarbageCollection.Debug {
				...
				err := config.K8sClient.NetworkingV1().NetworkPolicies(item.GetNamespace()).Delete(ctx, item.GetName(), k8smetav1.DeleteOptions{
...
func (c *Cleaner) Mark(ctx context.Context, namespace string, timestamp string) error {
	...
	for _, item := range resources {
		...
		np, err := utils.Transform[k8smetav1.Object, networkingv1.NetworkPolicy](item)
		if err != nil {
		...
		if _, err := config.K8sClient.NetworkingV1().NetworkPolicies(item.GetNamespace()).Update(ctx, &np, k8smetav1.UpdateOptions{}); err != nil {
			...
			alerting.RaiseAlert(ctx, alerting.ProcessLabeling, alerting.ErrorLabeling, c.ResourceType, &np)
