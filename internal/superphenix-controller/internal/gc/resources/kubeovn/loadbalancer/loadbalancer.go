...
func (c *Cleaner) Clean(ctx context.Context) error {
	...
	watcher := informers.WatcherSet[informers.SwitchLBRules]
	...
	for _, item := range resources {
		...
		if ok {
			if config.Global.GarbageCollection.Debug {
				...
				err := config.KubeOvnClient.KubeovnV1().SwitchLBRules().Delete(ctx, item.GetName(), k8smetav1.DeleteOptions{
...
func (c *Cleaner) Mark(ctx context.Context, namespace string, timestamp string) error {
	...
	for _, item := range resources {
		...
		lb, err := utils.Transform[k8smetav1.Object, v1.SwitchLBRule](item)
		if err != nil {
		...
		if _, err := config.KubeOvnClient.KubeovnV1().SwitchLBRules().Update(ctx, &lb, k8smetav1.UpdateOptions{}); err != nil {
			...
			alerting.RaiseAlert(ctx, alerting.ProcessLabeling, alerting.ErrorLabeling, c.ResourceType, &lb)
