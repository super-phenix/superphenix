...
// generateConnectionSecret generates a connection secret for this cluster using the kubeconfig secret generated by talos-operator
func (r *Reconciler) generateConnectionSecret(ctx context.Context, cluster *operatorv1alpha1.Cluster) error {
	log := logf.FromContext(ctx)

	// Get talos-operator kubeconfig secret:
	kubeconfigSecret := &corev1.Secret{}
	err := r.Get(ctx, types.NamespacedName{Name: fmt.Sprintf("%s-kubeconfig", cluster.Name), Namespace: cluster.Namespace}, kubeconfigSecret)
	if err == nil {
		// Parse kubeconfig from secret data:
		var kubeconfig kubeconfig.KubectlConfig
		if err := kyaml.Unmarshal(kubeconfigSecret.Data["kubeconfig"], &kubeconfig); err != nil {
			return err
		}

		// Generate the connection secret:
		if cluster.Spec.Connection.SecretRef == nil {
			return fmt.Errorf("Missing connection secretRef")
		}
		...
		_, err := controllerutil.CreateOrUpdate(ctx, r.Client, connSecret, func() error {
			// Prepare secret data:
			data := make(map[string]string)
			data["caData"] = string(kubeconfig.Clusters[0].Cluster.CertificateAuthorityData)
			data["certData"] = string(kubeconfig.Users[0].User.ClientCertificateData)
			data["keyData"] = string(kubeconfig.Users[0].User.ClientKeyData)
			data["insecure"] = "true"
			connSecret.StringData = data
			return nil
		})
		if err != nil {
			return err
		...
		log.Info("Successfully created cluster connection secret")
	} else if errors.IsNotFound(err) {
		log.Info("talos-operator kubeconfig secret not found, cannot generate cluster connection secret yet")
	} else {
