...
Helm chart to install Superphenix system on workload, storage, and management clusters

## Deployment Model

This chart is designed to be flexible and supports different deployment scenarios based on the cluster's purpose.

### Application Modes

Each application defined in the `apps` section can have a `modes` list. This list specifies which `deploymentTopology` modes (such as "Hyperconverged", "DecoupledWorkload", "DecoupledStorage", etc.) the application should be deployed in.

- If an application's `modes` list is empty or not specified, it is considered eligible for deployment in any mode, provided it is enabled.
- If a list is provided, the application will only be deployed if the cluster's `deploymentTopology` matches one of the specified modes.

### Management Clusters

Special logic applies when a cluster is designated as a management cluster by setting `.Values.cluster.type` to `Management`.

- In a management cluster, the effective deployment mode is forced to `Management`.
- Only applications with `Management` included in their `modes` list are considered for deployment. All other applications are ignored.
- For an application to be deployed in a management cluster, it must also be enabled (`enabled: true`).

In non-management clusters, all enabled applications are evaluated based on their `modes` configuration. Applications without specified modes are deployed in all non-management cluster topologies.
			...
			<td><pre lang="json">
{
  ...
  "superphenix-api": {
    ...
    "namespace": "superphenix-system",
    "repoURL": "oci://ghcr.io/super-phenix/charts",
    "targetRevision": ""
  },
  "superphenix-console": {
    "automation": {
  ...
  "volume-replicator": {
    ...
    "targetRevision": "0.5.1"
			...
			<td>"Per application" settings</td>
		</tr>
		<tr>
			<td>apps.cilium</td>
			<td>object</td>
			<td><pre lang="json">
{
  ...
  "wave": "-100"
			...
			<td>Cilium is used as the CNI on storage clusters TODO: Maybe phase out Cilium and use Kube-OVN everywhere?</td>
		</tr>
		<tr>
			<td>apps.coredns</td>
			<td>object</td>
			...
			<td><pre lang="json">
{
  ...
  "targetRevision": "2.6.1"
			...
			<td>iDRAC Exporter configuration</td>
		</tr>
		<tr>
			<td>apps.kratos</td>
			<td>object</td>
			...
			<td><pre lang="json">
{
  ...
  "wave": "-100"
			...
			<td>Multus is used for multi-attached pods/VMs</td>
		</tr>
		<tr>
			<td>apps.postgres</td>
			<td>object</td>
			...
			<td><pre lang="json">
{
  ...
  "targetRevision": ""
			...
			<td>Superphenix API configuration</td>
		</tr>
		<tr>
			...
			<td><pre lang="json">
{
  ...
  "targetRevision": "0.2.1"
			...
			<td>Superphenix Console configuration</td>
		</tr>
		<tr>
			...
			<td><pre lang="json">
{
  ...
  "wave": "-10"
			...
			<td>Superphenix Controller installed on the AZs</td>
		</tr>
		<tr>
			...
			<td><pre lang="json">
{
  ...
  "targetRevision": "0.6.1"
			...
			<td>Talos Operator configuration</td>
		</tr>
		<tr>
			<td>apps.traefik.helm.values.accessLog</td>
			<td>object</td>
			<td><pre lang="json">
{
  "enabled": true
			...
			<td>Enable the access logs within the Traefik pods</td>
		</tr>
		<tr>
			<td>apps.traefik.helm.values.api</td>
			<td>object</td>
			<td><pre lang="json">
{
  "dashboard": false
			...
			<td>We do not use the dashboard by default to save on compute and RAM</td>
		</tr>
		<tr>
			<td>apps.traefik.helm.values.deployment</td>
			<td>object</td>
			<td><pre lang="json">
{
  "enabled": true,
  "kind": "DaemonSet"
			...
			<td>Deploy Traefik as a DaemonSet so that every node can receive traffic</td>
		</tr>
		<tr>
			<td>apps.traefik.helm.values.gateway</td>
			<td>object</td>
			<td><pre lang="json">
{
  "enabled": false
			...
			<td>Gateway API default gateway (we do not use it)</td>
		</tr>
		<tr>
			<td>apps.traefik.helm.values.global</td>
			<td>object</td>
			<td><pre lang="json">
{
  "checkNewVersion": false
			...
			<td>Disable logs signaling about new versions</td>
		</tr>
		<tr>
			<td>apps.traefik.helm.values.hostNetwork</td>
			<td>bool</td>
			<td><pre lang="json">
true
</pre>
</td>
			<td>We're deploying on baremetal (maybe replace by hostPort in .ports structure?)</td>
		</tr>
		<tr>
			<td>apps.traefik.helm.values.ingressClass</td>
			<td>object</td>
			<td><pre lang="json">
{
  "enabled": true
			...
			<td>Create a Traefik default ingress class</td>
		</tr>
		<tr>
			<td>apps.traefik.helm.values.ports</td>
			<td>object</td>
			<td><pre lang="json">
{
  "kaas-api-tls": {
  ...
  "websecure": {
    "port": 443
			...
			<td>Ports on which Traefik listens</td>
		</tr>
		<tr>
			<td>apps.traefik.helm.values.ports.kaas-api-tls</td>
			<td>object</td>
			<td><pre lang="json">
{
  "expose": {
  ...
  "protocol": "TCP"
			...
			<td>KaaS TLS API server (HAProxy)</td>
		</tr>
		<tr>
			<td>apps.traefik.helm.values.ports.kaas-https</td>
			<td>object</td>
			<td><pre lang="json">
{
  "expose": {
  ...
  "protocol": "TCP"
			...
			<td>KaaS HTTPS API server</td>
		</tr>
		<tr>
			<td>apps.traefik.helm.values.ports.kaas-konnectivity</td>
			<td>object</td>
			<td><pre lang="json">
{
  "expose": {
  ...
  "protocol": "TCP"
			...
			<td>KaaS Konnectivity server</td>
		</tr>
		<tr>
			<td>apps.traefik.helm.values.ports.web</td>
			<td>object</td>
			<td><pre lang="json">
{
  "port": 80
		...
		<tr>
			<td>apps.traefik.helm.values.ports.websecure</td>
			<td>object</td>
			<td><pre lang="json">
{
  "port": 443
		...
		<tr>
			<td>apps.traefik.helm.values.providers</td>
			<td>object</td>
			<td><pre lang="json">
{
  "kubernetesGateway": {
  ...
  "kubernetesIngressNGINX": {
    ...
    "watchIngressWithoutClass": true
			...
			<td>We're listening for ingress-nginx annotations to support previous deployments</td>
		</tr>
		<tr>
			<td>apps.traefik.helm.values.securityContext</td>
			<td>object</td>
			<td><pre lang="json">
{
  "capabilities": {
    ...
    "drop": [
      "ALL"
			...
			<td>Security context to bind on port < 1024 (80 and 443)</td>
		</tr>
		<tr>
			<td>apps.traefik.helm.values.service</td>
			<td>object</td>
			<td><pre lang="json">
{
  "type": "ClusterIP"
			...
			<td>Use a ClusterIP instead of a LoadBalancer, as we're deploying on baremetal</td>
		</tr>
		<tr>
			<td>apps.traefik.helm.values.updateStrategy</td>
			<td>object</td>
			<td><pre lang="json">
{
  "rollingUpdate": {
  ...
  "type": "RollingUpdate"
			...
			<td>Update strategy for hostNetwork deployment</td>
		</tr>
		<tr>
			<td>argocd</td>
			<td>object</td>
			<td><pre lang="json">
{
  ...
  "project": "default"
			...
			<td>ArgoCD application parameters</td>
		</tr>
		<tr>
...
false
			...
			<td>Cleanup resources on deletion (cascading deletion) for all applications and override "per application" setting</td>
		</tr>
		<tr>
			<td>cluster</td>
			<td>object</td>
			<td><pre lang="json">
{
  ...
  "type": ""
			...
			<td>Cluster on which the applications should be deployed</td>
		</tr>
		<tr>
			<td>disableAll</td>
			<td>bool</td>
...
false
			...
			<td>Disable all applications and override "per application" setting</td>
		</tr>
		<tr>
			<td>forceManual</td>
			<td>bool</td>
...
false
			...
			<td>Force manual sync for all applications and override "per application" setting</td>
		</tr>
	</tbody>
</table>
